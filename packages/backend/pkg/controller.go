...
func (c *Controller) GetPrivateKeys(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		c.Log.Error("error creating keys directory: " + err.Error())
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode("error creating keys directory: " + err.Error())
		return
	}
	...
	for _, f := range files {
		file, err := os.Open(filepath.Join(keysDir, f.Name()))
		if err != nil {
